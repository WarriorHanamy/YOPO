...
class YopoTrainer:
    def __init__(
        ...
        if use_wandb:
            ...
            run_name = wandb_name or os.path.basename(self.tensorboard_path)
            wandb.init(
    ...
    def train_one_epoch(self, epoch: int, total_progress):
        ...
        for step, (depth, pos, rot, obs_b, map_id) in enumerate(
            ...
            if step % inspect_interval == inspect_interval - 1:
                ...
                self.tensorboard_log.add_scalar(
                    'Train/ScoreLoss',
                    np.mean(score_losses),
                    global_step,
                )
                self.tensorboard_log.add_scalar(
                    'Detail/SmoothLoss',
                    np.mean(smooth_losses),
                    global_step,
                )
                self.tensorboard_log.add_scalar(
                    'Detail/SafetyLoss',
                    np.mean(safety_losses),
                    global_step,
                )
                self.tensorboard_log.add_scalar(
                    'Detail/GoalLoss',
                ...
                if self._wandb:
                    self._wandb.log(
                        {
                            'Train/TrajLoss': np.mean(traj_losses),
                            'Train/ScoreLoss': np.mean(score_losses),
                            'Detail/SmoothLoss': np.mean(smooth_losses),
                            'Detail/SafetyLoss': np.mean(safety_losses),
                            'Detail/GoalLoss': np.mean(goal_losses),
                            'Detail/AccelLoss': np.mean(acc_losses),
                            'Train/BatchFPS': batch_fps,
                        },
                        step=global_step,
                    )
                (
    ...
    def eval_one_epoch(self, epoch: int):
        ...
        if self._wandb:
            self._wandb.log(
                {
                    'Eval/TrajLoss': np.mean(traj_losses),
                    'Eval/ScoreLoss': np.mean(score_losses),
                },
                step=epoch,
            )
        self.progress_log.remove_task(one_epoch_progress)
