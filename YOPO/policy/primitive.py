...
class LatticeParam:
    def __init__(self, train: bool = True):
        self._train = train
        ratio = 1.0 if train else config.velocity / config.vel_max_train
        self.vel_max = ratio * config.vel_max_train
        self.acc_max = ratio * ratio * config.acc_max_train
        self.segment_time = config.sgm_time / ratio
        self.horizon_num = config.lattice.horizon_num
        self.vertical_num = config.lattice.vertical_num
        self.radio_num = config.lattice.radio_num
        self.traj_num = config.traj_num
        self.horizon_fov = config.lattice.horizon_camera_fov
        self.vertical_fov = config.lattice.vertical_camera_fov
        self.horizon_anchor_fov = config.lattice.horizon_anchor_fov
        self.vertical_anchor_fov = config.lattice.vertical_anchor_fov
        self.radio_range = config.lattice.radio_range
...
class LatticePrimitive(LatticeParam):
        ...
        for h in range(0, self.radio_num):
            for i in range(0, self.vertical_num):
                for j in range(0, self.horizon_num):
                    ...
                    alpha = torch.tensor(
                        -direction_diff * (self.horizon_num - 1) / 2 + j * direction_diff
                    )
                    beta = torch.tensor(
                        -altitude_diff * (self.vertical_num - 1) / 2 + i * altitude_diff
                    )
    ...
    @classmethod
    def get_instance(cls, train: bool = True):
        if cls._instance is None:
            cls._instance = cls(train=train)
