...
Total primitives: `traj_num = horizon_num × vertical_num × radio_num = 5 × 3 × 1 = 15` (see `YOPO/schema.py:228` computed property).

### 3.2 Anchor Angle Computation (Deterministic)

See `YOPO/policy/primitive.py:47-56` for `direction_diff`, `altitude_diff`, `radio_diff` computation.
...
See `YOPO/policy/primitive.py:62-77` for the triple-nested loop that computes `(alpha, beta, search_radio)` per anchor and the spherical-to-Cartesian `pos_node`.
...
See `YOPO/policy/primitive.py:76-77` for ZYX Euler rotation `[alpha, -beta, 0]` building `R_bp`.
...
See `YOPO/policy/poly_solver.py:6-15` for the closed-form `Coef_inv` matrix mapping boundary conditions to coefficients.
...
End position is computed by converting `(yaw+delta_yaw, pitch+delta_pitch, radio)` from spherical to Cartesian in body frame (see `YOPO/policy/state_transform.py:29-33`). End velocity and acceleration are denormalized in primitive frame, then rotated to body frame via `Rbp` (see `YOPO/policy/state_transform.py:37-47`).

### 4.5 Segment Time

Segment time is computed as `sgm_time = 2 × radio_range / vel_max_train` (see `YOPO/schema.py:222` computed property). At test time, adjusted for speed ratio as `cfg["sgm_time"] / ratio` where `ratio = velocity / vel_max_train` (see `YOPO/policy/primitive.py:11`).
...
| Segment time | `T` | **1.667 s** | `schema.py:222`, `2×5.0/6.0` |
| Number of primitives | `traj_num` | **15** (3×5×1) | `schema.py:228` |
...
Body ↔ World transforms via `scipy.spatial.transform.Rotation` with ZYX Euler convention. See `YOPO/policy/state_transform.py:120-144` for `rotate_body2world` / `transform_body2world`.
